...
CryptoQuant AI is a local-first quantitative cryptocurrency trading dashboard for research, monitoring, backtesting, and optional Binance execution. It combines a React/Vite frontend with an Express/TypeScript backend, local SQLite persistence, Binance market/account integrations, Zhipu GLM-assisted summaries, and risk controls for strategy evaluation.

> This project is for research and personal operations. It is not financial advice. Use testnet trading first, keep API permissions limited, and never expose this service directly to the public internet without proper hardening.

## Features

- Binance market data, account balance, positions, order history, and order submission support.
- Testnet and live Binance credential paths with a local encrypted credential store.
...
- npm 10 or newer.
- Git.
- Optional Binance API credentials for live or testnet trading.
- Optional Zhipu GLM API key for AI-assisted summaries.
- Optional FRED API key for macro data.
- Optional local HTTP/SOCKS proxy if your network cannot reach Binance directly.
...
- `BINANCE_API_KEY`, `BINANCE_SECRET_KEY` for Binance live mode.
- `BINANCE_TESTNET_API_KEY`, `BINANCE_TESTNET_SECRET_KEY` for Binance USD-M futures testnet mode.
...
- Start with Binance testnet trading before live trading.
